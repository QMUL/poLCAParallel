...
#'   the manifest variables. If `TRUE`, those cases are removed (listwise
#'   deleted) before estimating the model. If `FALSE`, cases with missing values
#'   are retained. Cases with missing covariates are always removed. The default
#'   is `TRUE`.
...
#'   `nrep>1`, then any user-specified `probs.start` values are only used in the
#'   first of the `nrep` attempts.
...
#'   statistics, the chi squared statistics and the log likelihood ratio. The
#'   default is `TRUE`.

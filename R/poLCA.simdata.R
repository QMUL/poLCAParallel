...
#'     matrix corresponding to one manifest variable, and each row containing
#'     the class-conditional outcome probabilities (which must sum to 1). If
#'     `probs` is `NULL` (default) then the outcome probabilities are generated
#'     randomly.
#' @param nclass number of latent classes. If `probs` is specified, then
#'     `nclass` is set equal to the number of rows in each matrix in that list.
#'     If `P` is specified, then `nclass` is set equal to the length of that
#'     vector. If `b` is specified, then `nclass` is set equal to one greater
#'     than the number of columns in `b`. Otherwise, the default is two.
...
#'     specified, then `ndv` is set equal to the length of that vector.
#'     Otherwise, the default is four.
...
#'     If `x=NULL` (default), but  `niv>0`, then `niv` concomitant variables
#'     will be generated as mutually independent random draws from a standard
#'     normal distribution.
...
#'     then the number of columns in `x` overrides the value of `niv`. The
#'     number of rows in `b`, less one, also overrides `niv`.
...
#' - `probs`: a list of matrices of dimension `nclass` by `nresp`
#'   containing the class-conditional response probabilities.
...
#' probs <- list(matrix(c(0.6,0.1,0.3,     0.6,0.3,0.1,     0.3,0.1,0.6    ),
...
#'                      ncol = 3, byrow = TRUE)) # conditional resp prob to Y5
...
#' simdat2 <- poLCA.simdata(N = 1000, ndv = 7, niv = 3, nclass = 2,
#'                          b=matrix(c(1,-2,1,-1)))
